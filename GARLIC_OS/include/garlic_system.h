...
extern int
    _gd_sincMain; // Sincronismos con programa principal:
...
extern garlicSPRITE
    _gd_sprites[16 * 8]; // vector amb els 128 (16 * 8) PCBs de sprites
...
/* intFunc:		nuevo tipo de dato para representar puntero a función
   que devuelve un int, concretamente, el puntero a la función de inicio de los
...
/* _gp_numProc:	devuelve el número de procesos cargados en el sistema,
                                incluyendo el proceso en RUN y los procesos en
   READY */
extern int _gp_numProc();

/* _gp_crearProc:	prepara un proceso para ser ejecutado, creando su
   entorno de ejecución y colocándolo en la cola de READY; Parámetros: funcion
   ->	dirección de memoria de entrada al código del proceso zocalo	->
   identificador del zócalo (0 - 15) nombre	->	nombre en clave del
   programa (4 caracteres ASCII) arg		->	argumento del programa
...
/* _gp_retardarProc:	retarda la ejecuci?n del proceso actual durante el
                                n?mero de segundos que se especifica por
   par?metro, coloc?ndolo en el vector de DELAY; Par?metros: nsec ->	n?mero
   de segundos (m?x. 600); si se especifica 0, el proceso solo se desbanca y el
   retardo ser? el tiempo que tarde en ser restaurado (depende del n?mero de
   procesos activos del sistema) ATENCI?N: ?el proceso del sistema operativo
   (PIDz = 0) NO podr? utilizar esta funci?n, para evitar que el procesador se
   pueda quedar sin procesos a ejecutar!
*/
extern int _gp_retardarProc(int nsec);

/* _gp_matarProc:	elimina un proceso de las colas de READY o DELAY, seg?n
                                donde se encuentre, libera memoria y borra el
   PID de la estructura _gd_pcbs[zocalo] correspondiente al z?calo que se pasa
   por par?metro; ATENCI?N:	Esta funci?n solo la llamar? el sistema
   operativo, por lo tanto, no ser? necesario realizar comprobaciones del
   par?metro; por otro lado, el proceso del sistema operativo (zocalo = 0) ?NO
   se tendr? que destruir a s? mismo!
*/
extern int _gp_matarProc(int zocalo);

/* _gp_rsiTIMER0:	servicio de interrupciones del TIMER0 de la plataforma
   NDS, que refrescar? peri?dicamente la informaci?n de la tabla de procesos
   relativa al tanto por ciento de uso de la CPU; */
...
/* _gm_initFS: inicializa el sistema de ficheros, devolviendo un valor booleano
                                        para indiciar si dicha inicialización ha
   tenido éxito;
*/
extern int _gm_initFS();

/* _gm_listaProgs: devuelve una lista con los nombres en clave de todos
                                los programas que se encuentran en el directorio
   "Programas". Se considera que un fichero es un programa si su nombre tiene 8
   car?cteres y termina con ".elf"; se devuelven solo los 4 primeros car?cteres
   del nombre del fichero (nombre en clave), que por convenio deben estar en
   may?sculas; el resultado es un vector de strings (paso por referencia) y el
   n?mero de programas detectados; */
extern int _gm_listaProgs(char *progs[]);

/* _gm_cargarPrograma: busca un fichero de nombre "(keyName).elf" dentro del
                                        directorio "/Programas/" del sistema de
   ficheros y carga los segmentos de programa a partir de una posici?n de
                                        memoria libre, efectuando la reubicaci?n
   de las referencias a los s?mbolos del programa seg?n el desplazamiento del
                                        c?digo y los datos en la memoria
   destino; Par?metros: zocalo	->	?ndice del z?calo que indexar? el
   proceso del programa keyName ->	string de 4 car?cteres con el nombre en
   clave del programa Resultado:
...
/* _gm_do_malloc: funció auxiliar 1 progM per reservar memòria dinàmica per a un
 * procés */
extern void *_gm_do_malloc(unsigned int size, int zocalo);

/* _gm_do_free: funció auxiliar 2 progM per alliberar memòria dinàmica d'un
 * procés */
...
/* _gm_reubicar: rutina de soporte a _gm_cargarPrograma(), que interpreta los
                                        'relocs' de un fichero ELF contenido en
   un buffer *fileBuf, y ajusta las direcciones de memoria correspondientes a
   las referencias de tipo R_ARM_ABS32, a partir de las direcciones de memoria
   destino de c?digo (dest_code) y datos (dest_data) y seg?n el valor de las
   direcciones de las referencias a reubicar y de las direcciones de inicio de
   los segmentos de c?digo (pAddr_code) y datos (pAddr_data); */
// extern void _gm_reubicar(char *fileBuf, unsigned int pAddr_code,
//                          unsigned int *dest_code, unsigned int pAddr_data,
...
/* _gm_reservarMem: rutina para reservar un conjunto de franjas de memoria
                                libres consecutivas que proporcionen un espacio
   suficiente para albergar el tama?o de un segmento de c?digo o datos del
   proceso (seg?n indique tipo_seg), asignando al n?mero de z?calo que se pasa
   por par?metro; la rutina devuelve la primera direcci?n del espacio reservado;
                                en el caso de que no quede un espacio de memoria
   consecutivo del tama?o requerido, devuelve cero; */
...
/* _gm_rsiTIMER1:	servicio de interrupciones del TIMER1 de la plataforma
   NDS, que refrescar? peri?dicamente la informaci?n de la tabla de procesos
   relativa al uso de la pila y el estado del proceso; */
...
/* _gg_escribir: escribe una cadena de caracteres en la ventana indicada;
        Parámetros:
                formato	->	cadena de formato, terminada con centinela '\0';
                                        admite '\n' (salto de línea), '\t'
   (tabulador, 4 espacios) y códigos entre 32 y 159 (los 32 últimos son
   caracteres gráficos), además de marcas de format %c, %d, %h y %s (max. 2
   marcas por cadena) val1	->	valor a sustituir en la primera marca de
   formato, si existe val2	->	valor a sustituir en la segunda marca de
   formato, si existe
                                        - los valores pueden ser un código ASCII
   (%c), un valor natural de 32 bits (%d, %x) o un puntero a string (%s) ventana
   ->	número de ventana (de 0 a 3)
*/
...
/* _gg_escribirLinea: rutina de soporte a _gg_escribir(), para escribir sobre la
                                        fila (f) de la ventana (v) los caracters
   pendientes (n) del buffer de ventana correspondiente.
*/
extern void _gg_escribirLinea(int v, int f, int n);

/* desplazar: rutina de soporte a _gg_escribir(), para desplazar una posición
                                        hacia arriba todas las filas de la
...
/* _gg_escribirCar: escribe un car?cter (baldosa) en la posici?n de la ventana
        ...
        Par?metros:
                ...
                c		->	c?digo del car?cter, como n?mero de
   baldosa (0..127) color	->	color del texto (0..3) ventana	->
   n?mero de ventana (0..15)
*/
...
/* _gg_escribirMat: escribe una matriz de 8x8 car?cteres a partir de una
                                posici?n de la ventana indicada, con un color
   concreto; Par?metros: vx		->	coordenada x inicial de ventana
   (0..31) vy		->	coordenada y inicial de ventana (0..23) m
   ->	matriz 8x8 de c?digos ASCII color	->	color del texto (0..3)
...
/* _gg_escribirLineaTabla: escribe los campos b?sicos de una l?nea de la tabla
                                de procesos, correspondiente al n?mero de z?calo
   que se pasa por par?metro con el color especificado; los campos a escribir
   son: n?mero de z?calo, PID y nombre clave del proceso (keyName);
*/
extern void _gg_escribirLineaTabla(int z, int color);

/* _gg_rsiTIMER2:	servicio de interrupciones del TIMER2 de la plataforma
   NDS, que refrescar? peri?dicamente la informaci?n de la tabla de procesos
   relativa a la direcci?n actual de ejecuci?n; */
...
/* _gs_num2str_dec: convierte el número pasado por valor en el parámetro num
                                        a una representación en códigos ASCII de
   los dígitos decimales correspondientes, escritos dentro del vector de
                                        caracteres numstr, que se pasa por
   referencia; el parámetro length indicará la longitud del vector; la rutina
   coloca un caracter centinela (cero) en la última posición del vector
                                        (numstr[length-1]) y, a partir de la
   penúltima posición, empieza a colocar los códigos ASCII correspondientes a
   las unidades, decenas, centenas, etc.; en el caso que después de trancribir
   todo el número queden posiciones libres en el vector, la rutina rellenará
   dichas posiciones con espacios en blanco, y devolverá un cero; en el caso que
   NO hayan suficientes posiciones para transcribir todo el número, la función
   abandonará el proceso y devolverá un valor diferente de cero. ATENCIóN:
   solo procesa números naturales de 32 bits SIN signo. */
extern int _gs_num2str_dec(char *numstr, unsigned int length, unsigned int num);

/* _gs_num2str_hex:	convierte el parámetro num en una representación en
   códigos ASCII sobre el vector de caracteres numstr, en base 16 (hexa),
   siguiendo las mismas reglas de gestión del espacio del string que
   _gs_num2str_dec, salvo que las posiciones de más peso vacías se rellenarán
   con ceros, no con espacios en blanco */
...
/* _gs_copiaMem: copia un bloque de numBytes bytes, desde una posición de
                                memoria inicial (*source) a partir de otra
   posición de memoria destino (*dest), teniendo en cuenta que ambas posiciones
   de memoria deben estar alineadas a word */
extern void _gs_copiaMem(const void *source, void *dest, unsigned int numBytes);

/* _gs_borrarVentana: borra el contenido de la ventana que se pasa por
   par?metro, as? como el campo de control del buffer de ventana
                                _gd_wbfs[ventana].pControl; la rutina puede
   operar en una configuraci?n de 4 o 16 ventanas, seg?n el par?metro de modo;
        Par?metros:
...
/* _gs_escribirStringSub: escribe un string (terminado con centinela cero) a
                                partir de la posici?n indicada por par?metros
   (fil, col), con el color especificado, en la pantalla secundaria; */
...
/* _gs_pintarFranjas: rutina para pintar las l?neas verticales correspondientes
                                a un conjunto de franjas consecutivas de memoria
   asignadas a un segmento (de c?digo o datos) del z?calo indicado por
   par?metro. Par?metros: zocalo		->	el z?calo que reserva la
   memoria (0 para borrar) index_ini	->	el ?ndice inicial de las franjas
                num_franjas	->	el n?mero de franjas a pintar
                tipo_seg	->	el tipo de segmento reservado (0 ->
   c?digo, 1 -> datos)
*/
...
/* _gs_representarPilas: rutina para para representar gr?ficamente la ocupaci?n
                                de las pilas de los procesos de usuario, adem?s
   de la pila del proceso de control del sistema operativo, sobre la tabla de
                                control de procesos.
*/
...
/* _gi_movimientoVentanas:	actualiza el desplazamiento y escalado de los
   ...
   efectuar los movimientos de las ventanas seg?n el comportamiento requerido de
...
/* _gi_redibujarZocalo: rutina para actualizar la tabla de z?calos en funci?n
                                del z?calo actual (_gi_za) y del par?metro
   (seleccionar): si seleccionar == 0, dibuja la l?nea de _gi_za seg?n el color
   asociado al estado del z?calo (blanco -> activo, salm?n -> libre); sino,
   dibuja la l?nea en magenta;
*/
